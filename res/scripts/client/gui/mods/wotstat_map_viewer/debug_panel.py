...
def _validValue(control, value):
  ...
  if kind == 'timeline':
    return (
      ...
      and set(value) == set(('position', 'playing'))
      and _number(value['position'])
      and 0 <= value['position'] <= 100
...
def _control(data):
  ...
  if data.get('type') == 'text':
    ...
    if (not isinstance(color, (int, long)) or isinstance(color, bool)
        or not 0 <= color <= 0xFFFFFF):
      raise ValueError('Text color must be an RGB integer')

    return dict(
      id=_text(data.get('id'), 'control id'),
      type='text',
  ...
  result = dict(
    id=_text(data.get('id'), 'control id'),
    label=_text(data.get('label'), 'control label'),
    type=data.get('type'),
    tooltip=tooltip,
  )
  ...
  if result['type'] == 'slider':
    ...
    if (not all(_number(v) for v in (low, high, step))
        or low >= high or step <= 0):
    ...
    for option in options:
      ...
      result['options'].append(dict(
        label=_text(option.get('label'), 'option label'),
        value=value,
      ))
...
class SettingsRegistry(object):
  ...
  def snapshot(self):
    return [
      dict(
        ...
        controls=copy.deepcopy(section['controls'].values()),
      )
  ...
  def changeValue(self, sectionID, controlID, value):
    ...
    if section is None or controlID not in section['controls']: return False
