...
class LocalArenaBorder(ArenaBorderController):
  ...
  def attach(self, spaceID, boundingBox):
    ...
    self._applySetting(
      self.settingsCore.getSetting(settings.BATTLE_BORDER_MAP.MODE_SHOW_BORDER),
      self.settingsCore.getSetting(settings.BATTLE_BORDER_MAP.TYPE_BORDER),
      self._ArenaBorderController__getCurrentColor(
        self.settingsCore.getSetting(settings.GRAPHICS.COLOR_BLIND)))

  def extended(self, isDown):
    self._ArenaBorderController__handleShowExtendedInfo(
      GameEvent(GameEvent.SHOW_EXTENDED_INFO, {'isDown': isDown}))
