...
class LocalSession(object):
  ...
  def _ensureNativeHangarConfig(self, path):
    ...
    key = path.lower()
    if key in ClientHangarSpace._HANGAR_CFGS: return
  ...
  def _openView(self):
    ...
    try:
      ...
      cleanup.defer(
        'restore lobby optimizer',
        lobbyApp.graphicsOptimizationManager.switchOptimizationEnabled,
        self._saved['optimizer'])
      ...
      if not self._nativeHangar:
        self.mappingID = BigWorld.addSpaceGeometryMapping(
          self.spaceID, None, 'spaces/' + self.arena.geometryName, mask)
      ...
      log.info(
        'Loading geometry=%s mode=%s space=%s; Account retained',
        self.arena.geometryName, self.arena.gameplayName, self.spaceID)
    except Exception:
  ...
  def _tick(self):
    ...
    self._callback = None
    if not self.active or self.stopping: return
    ...
    try:
      ...
      if (not self._hudRequested
          and BigWorld.spaceLoadStatus() >= 0.99
  ...
  def handleMouse(self, event):
    if self.stopping or self.hud is None: return True
  ...
  def handleKey(self, event, original):
    ...
    if event.key in (Keys.KEY_LCONTROL, Keys.KEY_RCONTROL):
      if event.isRepeatedEvent(): return True
    ...
    if event.key == Keys.KEY_ESCAPE:
      if not event.isKeyDown() or event.isRepeatedEvent(): return True
